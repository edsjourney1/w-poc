...
function decorateButtons(element) {
  element.querySelectorAll('a').forEach((a) => {
    ...
    if (a.href !== a.textContent) {
      ...
      if (!a.querySelector('img') && a.href.indexOf('urn:aaid:aem:') < 0) {
...
async function loadBlock(block) {
  ...
  if (status !== 'loading' && status !== 'loaded') {
    ...
    const exclusions = [
      'blocks/adhoc-modal',
      'blocks/cobranded-modal',
      'blocks/external-popup',
      'blocks/language-helper',
      'blocks/medicare-modal',
    ];
    let shouldProceed = true;
    exclusions.forEach((ex) => {
      if (ex.includes(blockName) && shouldProceed) {
        shouldProceed = false;
      }
    });
    try {
      const cssLoaded =
        shouldProceed && loadCSS(`${window.hlx.codeBasePath}/blocks/${blockName}/${blockName}.css`);
      const decorationComplete = new Promise((resolve) => {
        (async () => {
          try {
            const mod =
              shouldProceed &&
              (await import(`${window.hlx.codeBasePath}/blocks/${blockName}/${blockName}.js`));
            if (mod.default) {
