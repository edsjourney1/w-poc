...
export default async function decorate(block) {
  ...
  if (block.children[2].children[0].innerText.trim().toLowerCase() === 'category') {
    categoryByauthor = block.children[2].children[1].innerText;
    categoryBasedJson = json.data.filter((article) =>
      article.category.toLowerCase().includes(categoryByauthor.toLowerCase()),
    );
  }
  if (block.children[2].children[0].innerText.trim().toLowerCase() === 'tag') {
    tagByauthor = block.children[2].children[1].innerText;
    categoryBasedJson = json.data.filter((article) =>
  ...
  function renderItems() {
    ...
    sortedJSON.forEach(({ image, tags, publishedDate, readTime, title, url, segment }) => {
  ...
  if (categoryBasedJson.length > 10) {
    renderPagination();
  } else {
    renderItems();
  }
