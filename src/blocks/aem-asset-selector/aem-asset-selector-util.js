...
const IMS_ENV_STAGE = 'Stage';
const IMS_ENV_PROD = 'Production';
...
const IMS_CLIENT_ID = 'aemcs-wellmark-assetselector';
const ASSET_SELECTOR_ID = 'asset-selector';
const CLIPBOARD_SUPPORTED_BINARY_MIMETYPES = ['image/png'];
const SUPPORTED_RENDITIONS_FORMATS = ['image/png', 'image/jpeg'];
...
export function init(cfg, callback) {
  if (cfg.environment) {
    ...
    } else if (cfg.environment.toUpperCase() === 'PRODUCTION') {
...
function getCopyRendition(asset) {
  ...
  renditions
    ...
    .forEach((rendition) => {
      if (
        SUPPORTED_RENDITIONS_FORMATS.includes(rendition.type) &&
...
export async function copyAssetWithRapi(asset) {
  ...
  try {
    ...
    if (!res.ok) {
      console.log(
        `Download request for rendition binary failed with status code ${res.status}: ${res.statusText}`,
      );
      return false;
    }
...
export async function renderAssetSelectorWithImsFlow(cfg) {
  const assetSelectorProps = {
    ...
    env: cfg.environment ? cfg.environment.toUpperCase() : 'PRODUCTION',
    apiKey: API_KEY,
    hideTreeNav: true,
